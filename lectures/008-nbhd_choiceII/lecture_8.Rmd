...
--
.smaller[
- __Houses go to highest RP group__

   - Means either highest WTP or consumer surplus leads to bargaining amongst households
...
ggplot(data.frame(x=c(0, 8)), aes(x)) +
  ...
  labs(x = "number of high income HHs in neighborhood A",y = "Rent premium = R(A)-R(B)")+
...
- If 5 high income HHs move into A, $RP(high) > RP(low)$
...
- More highs move in (and since they bid higher, they get to live in neighborhood A). This is called a _ self-reinforcing effect_ or positive feedback loop. 
...
- Now, a small movement of high-income HHs into A means $RP(High) < RP(low)$
...
```
.hi.orange[Note]: 80 high income HHs in A is not an EQ because $RP(low)>RP(high)$. So low incomes will outbid highs and move in
...
--
  ...
  - That is, an eq is unstable if when you move away from it, the population dynamics push you even farther than where you came from
...
4) For every eq. value, look at its immediate vicinity
  ...
  - If arrows are moving towards it, it is a .hi.purple[stable eq]
  
  - If arrows are moving away from it on one or both sides, it is a .hi[unstable eq]
...
The model we just covered assumed that people sort entirely based on personal preferences (essentially). We ignored:
  ...
  - Information constraints (may vary across demographic groups)
...
.hi[De-facto] segregation: Results from choices made by private individuals (and not deliberate actions by institutions)
...
.hi[1945] WW2 is over. Americans are coming home from Europe
   ...
   - .hi.purple[Europe also has a massive refuge crisis (unfortunately we don't have time for this)]

FHA insured (guaranteed) home loans for banks. Except they explicitly did so on the condition that homes were not sold to African Americans
...
__Levittown, PA__ was a large development built by [Levitt & Sons]. 
  ...
  - FHA guaranteed Levitt & Sons  demand by offering qualified veterans subsidies on housing
  
  - FHA explicitly included racial covenants, and Levitt & Sons refused to sell homes to people of color.
...
2) .hi[De Facto vs De Jure]  `r emo::ji("check")`
...
1) White households have a preference for segregated neighborhoods 
...
.hi[Central City Schools]
  ...
  - twice high school dropout rate
